...
export const CadViewport: React.FC<CadViewportProps> = ({
  ...
  // Build / Update Parametric 3D CAD Geometry with SIMP Topology Evolution and FEA Stress Colormap
  useEffect(() => {
    ...
    // Current SIMP optimization progression ratio: 0 (initial block) -> 1 (converged organic truss)
    const progress = optimizationState.totalIterations > 0
      ? optimizationState.currentIteration / optimizationState.totalIterations
      : 0;

    // Material definitions based on selection
    const baseColor = new THREE.Color(selectedMaterial.color);
    const clipPlanes = showSection && clipPlaneRef.current ? [clipPlaneRef.current] : [];

    // Helper: Colormap from Von Mises stress (0 to 1 normalized)
    const getStressColor = (normStress: number): THREE.Color => {
      // Blue -> Cyan -> Green -> Yellow -> Red
      const color = new THREE.Color();
      if (normStress < 0.25) {
        color.setHSL(0.6 - normStress * 0.4, 0.9, 0.5); // Blue to Cyan
      } else if (normStress < 0.5) {
        color.setHSL(0.5 - (normStress - 0.25) * 0.6, 0.9, 0.5); // Cyan to Green
      } else if (normStress < 0.75) {
        color.setHSL(0.35 - (normStress - 0.5) * 0.8, 1.0, 0.5); // Green to Yellow
      } else {
        color.setHSL(0.15 - (normStress - 0.75) * 0.6, 1.0, 0.5); // Yellow to Red
      }
      return color;
    };

    // 1. BASE MOUNTING LUGS (Preserved Non-Design Regions, ρ = 1.0)
    // Left lug
    const leftLugGeo = new THREE.CylinderGeometry(14, 14, 18, 32);
    leftLugGeo.rotateZ(Math.PI / 2);
    leftLugGeo.translate(-45, -25, -20);
    const leftLugMat = new THREE.MeshStandardMaterial({
      color: activeTab === 'analizis' ? 0x38bdf8 : 0x7bd0ff,
      metalness: 0.7,
      roughness: 0.3,
      clippingPlanes: clipPlanes,
      clipShadows: true,
    });
    const leftLug = new THREE.Mesh(leftLugGeo, leftLugMat);
    modelGroup.add(leftLug);

    // Right lug
    const rightLugGeo = new THREE.CylinderGeometry(14, 14, 18, 32);
    rightLugGeo.rotateZ(Math.PI / 2);
    rightLugGeo.translate(45, -25, -20);
    const rightLugMat = leftLugMat.clone();
    const rightLug = new THREE.Mesh(rightLugGeo, rightLugMat);
    modelGroup.add(rightLug);

    // Rear base plate
    const basePlateGeo = new THREE.BoxGeometry(110, 16, 25);
    basePlateGeo.translate(0, -25, -20);
    const basePlateMat = new THREE.MeshStandardMaterial({
      color: activeTab === 'analizis' ? 0x10b981 : 0x89ceff,
      metalness: 0.7,
      roughness: 0.35,
      clippingPlanes: clipPlanes,
    });
    ...
    // Top loading eyelet (Preserved cylindrical non-design space, ρ = 1.0)
    const eyeletGeo = new THREE.CylinderGeometry(16, 16, 22, 32);
    eyeletGeo.rotateX(Math.PI / 2);
    eyeletGeo.translate(0, 38, 30);
    const eyeletMat = new THREE.MeshStandardMaterial({
      color: activeTab === 'analizis' ? 0xef4444 : 0x7bd0ff,
      metalness: 0.8,
      roughness: 0.25,
      clippingPlanes: clipPlanes,
    });
    const eyelet = new THREE.Mesh(eyeletGeo, eyeletMat);
    modelGroup.add(eyelet);

    // Hole inside eyelet (Keep-Out void cylinder, ρ = 0.0)
    const eyeletHoleGeo = new THREE.CylinderGeometry(8, 8, 26, 32);
    eyeletHoleGeo.rotateX(Math.PI / 2);
    eyeletHoleGeo.translate(0, 38, 30);
    const eyeletHoleMat = new THREE.MeshBasicMaterial({
      color: 0x0b0e17,
      wireframe: true,
    });
    const eyeletHole = new THREE.Mesh(eyeletHoleGeo, eyeletHoleMat);
    modelGroup.add(eyeletHole);

    // 2. TOPOLOGY OPTIMIZED TRUSS BODY (SIMP organic structural arms)
    // When progress === 0, creates a solid bulk engineering bracket block.
    // When progress > 0, branches into optimized triangular lattice webs and struts connecting the rear lugs to the top eyelet!
    const strutCount = 18;
    const curveRadius = 8 - progress * 3.5; // Thins out from 8mm down to ~4.5mm
    const curveSegments = 24;

    // Define main load paths (trusses calculated by SIMP density gradient)
    const trussPaths: THREE.Curve<THREE.Vector3>[] = [
      // Left leg to eyelet
      new THREE.CatmullRomCurve3([
        new THREE.Vector3(-42, -22, -15),
        new THREE.Vector3(-28 - progress * 8, 0, 5),
        new THREE.Vector3(-14, 25, 20),
        new THREE.Vector3(-5, 36, 28),
      ]),
      // Right leg to eyelet
      new THREE.CatmullRomCurve3([
        new THREE.Vector3(42, -22, -15),
      ...
      new THREE.CatmullRomCurve3([
        ...
        new THREE.Vector3(0, 8 - progress * 6, 8),
        new THREE.Vector3(0, 34, 26),
      ]),
      // Diagonal lateral stiffeners (form during iterations)
      new THREE.CatmullRomCurve3([
        new THREE.Vector3(-35, -20, -12),
        new THREE.Vector3(0, 2 - progress * 4, 15),
        new THREE.Vector3(35, -20, -12),
      ]),
    ];

    // Build organic tube geometry for each truss
    trussPaths.forEach((curve, idx) => {
      const radius = idx === 2 ? curveRadius * 1.2 : curveRadius;
      const tubeGeo = new THREE.TubeGeometry(curve, curveSegments, radius, 16, false);

      // In FEA Analysis mode, compute vertex colors based on Von Mises stress formula
      if (activeTab === 'analizis') {
        ...
        for (let i = 0; i < count; i++) {
          const y = pos.getY(i);
          const z = pos.getZ(i);
          // Stress is highest near the eyelet (y > 20, z > 15) and corner stress concentrations
          const heightFactor = Math.max(0, Math.min(1, (y + 20) / 55));
          const notchFactor = Math.sin(heightFactor * Math.PI) * 0.3;
          let normStress = heightFactor * 0.7 + notchFactor;
          normStress = Math.min(0.95, Math.max(0.05, normStress));

          const c = getStressColor(normStress);
          colors[i * 3] = c.r;
          colors[i * 3 + 1] = c.g;
          colors[i * 3 + 2] = c.b;

          // Apply deformation displacement exaggeration if specified
          if (deformationScale > 1) {
            const defY = -(normStress * (deformationScale * 0.12));
            pos.setY(i, y + defY);
          }
        }
      ...
      const tubeMat = new THREE.MeshStandardMaterial({
        color: activeTab === 'analizis' ? 0xffffff : baseColor,
        vertexColors: activeTab === 'analizis',
        metalness: 0.65,
        roughness: 0.35,
        clippingPlanes: clipPlanes,
        clipShadows: true,
      ...
      // Wireframe overlay if Gmsh mesh is toggled
      if (showMesh) {
    ...
    // 3. SOLID INITIAL CONVEX BLOCK (visible when iterations are low / 0)
    if (progress < 0.85) {
      const solidAlpha = Math.max(0, 1 - progress * 1.2);
      if (solidAlpha > 0.05) {
        const bulkGeo = new THREE.CylinderGeometry(22, 55, 68, 6);
        bulkGeo.rotateX(Math.PI / 4);
        bulkGeo.translate(0, 5, 5);
        const bulkMat = new THREE.MeshStandardMaterial({
          color: 0x272a33,
          transparent: true,
          opacity: solidAlpha * 0.4,
          wireframe: true,
        });
    ...
    // 4. BOUNDARY CONDITION 3D GLYPHS
    // Fixation support glyphs (Cyan anchors at base lugs)
    const fixCondition = boundaryConditions.find((c) => c.type === 'fijacion' && c.active);
    if (fixCondition) {
      [-45, 45].forEach((xPos) => {
        // Base anchor cone
        const anchorGeo = new THREE.ConeGeometry(4, 10, 8);
        anchorGeo.rotateZ(Math.PI);
        anchorGeo.translate(xPos, -38, -20);
        ...
        // Ground anchor pad
        const padGeo = new THREE.BoxGeometry(16, 2, 16);
        padGeo.translate(xPos, -44, -20);
        const pad = new THREE.Mesh(padGeo, anchorMat);
        modelGroup.add(pad);
      });
    }

    // Load vector arrow (Amber arrow pointing down & forward at the eyelet)
    const loadCondition = boundaryConditions.find((c) => c.type === 'carga' && c.active);
    if (loadCondition) {
      const arrowOrigin = new THREE.Vector3(0, 55, 30);
      const arrowDir = new THREE.Vector3(0, -0.9, 0.24).normalize();
      const arrowLength = 28;
      const arrowColor = 0xffb95f; // Tertiary Amber

      const arrowHelper = new THREE.ArrowHelper(arrowDir, arrowOrigin, arrowLength, arrowColor, 7, 4);
      modelGroup.add(arrowHelper);

      // Force magnitude text sprite marker
      ...
      if (ctx) {
        ...
        ctx.fillText('4.5 kN', 10, 32);
      }
      const textTexture = new THREE.CanvasTexture(canvas);
      const spriteMat = new THREE.SpriteMaterial({ map: textTexture });
      const sprite = new THREE.Sprite(spriteMat);
      sprite.position.set(16, 56, 32);
      sprite.scale.set(18, 7, 1);
      modelGroup.add(sprite);
    }

    // Safe region visual highlight (Green rings)
    const safeCondition = boundaryConditions.find((c) => c.type === 'preservada' && c.active);
    if (safeCondition) {
      [-45, 45].forEach((xPos) => {
        const ringGeo = new THREE.TorusGeometry(15, 1.2, 8, 24);
        ringGeo.rotateY(Math.PI / 2);
        ringGeo.translate(xPos, -25, -20);
    ...
    // Keep-Out region (Red transparent envelope)
    const keepoutCondition = boundaryConditions.find((c) => c.type === 'keepout' && c.active);
    if (keepoutCondition) {
      const voidCylinderGeo = new THREE.CylinderGeometry(9, 9, 36, 16);
      voidCylinderGeo.rotateX(Math.PI / 2);
      voidCylinderGeo.translate(0, 38, 30);
      const voidCylinderMat = new THREE.MeshBasicMaterial({
        color: 0xef4444,
        transparent: true,
        opacity: 0.35,
        wireframe: true,
      });
      const voidCylinder = new THREE.Mesh(voidCylinderGeo, voidCylinderMat);
      modelGroup.add(voidCylinder);
    }
  }, [
    activeTab,
    selectedMaterial,
