...
Resumen de la auditoría y las soluciones aplicadas

prompts.md es un informe de auditoría (de una sesión previa de ChatGPT) sobre core/face_correspondence.py, que construye la correspondencia 1:1 entre caras CAD y superficies Gmsh. Detectaba dos problemas P0 reales y uno P1 de robustez limitada. Ya localicé el código exacto en el repo y aplicé las correcciones:

P0.2 — Bug de desalineación de índices en el muestreo UV (el más grave)
En _gmsh_surface_signatures(), los puntos que fallaban en getValue() se saltaban con continue, pero después se accedía a pts_arr[i*samples+j] asumiendo una cuadrícula completa. Reproduje el bug de forma aislada: con un solo fallo intermedio, esto provoca un IndexError (o, en cuadrículas más grandes donde no revienta, asocia puntos de posiciones distintas → área y firma incorrectas).

Solución: ahora uso una matriz grid[samples, samples, 3] con máscara valid[i,j], así cada muestra fallida deja un "hueco" en su posición exacta en vez de desplazar las siguientes. El área solo se calcula en celdas cuyas 4 esquinas son válidas (subestima el área de forma segura en vez de corromperla), y si la cobertura cae por debajo del 50%, la superficie se marca explícitamente como degradada (firma (0,0,0)/(0,0,1)/area=0 + warning), igual que ya se hacía en el lado CAD.

P0.1 — Firma geométrica insuficiente para caras simétricas/congruentes
Centro + normal + área no distingue caras con la misma posición/orientación/área pero forma distinta (ej. rectángulo vs. óvalo de igual área). Agregué un cuarto descriptor: la extensión principal en el plano de la cara (autovalores de la matriz de covarianza de los puntos muestreados, proyectados sobre el plano tangente — descarto la componente normal para no mezclar curvatura con forma). Se calcula tanto para caras CAD (via face.tessellate()) como para superficies Gmsh (reutilizando la grilla ya muestreada), y se suma a _signature_distance. Caras verdaderamente congruentes seguirán empatando (correcto: deben seguir disparando AmbiguousFaceCorrespondenceError en vez de adivinar), pero caras con formas distintas que antes podían empatar por casualidad ahora quedan mejor separadas.

P1 — Fallback boundary (no lo toqué, es una limitación aceptada y ya señalizada)
Está duplicado en generative_engine.py y kratos_adapter.py: exige que los 3 nodos de un triángulo pertenezcan al conjunto de nodos de la cara, lo cual puede descartar triángulos legítimos cerca de bordes compartidos. El propio código ya lo trata como aproximación y emite warning cuando no encuentra triángulos. Si quieres, puedo relajarlo a "mayoría de nodos" (2 de 3) con voto de desempate, pero eso cambia semántica de asignación de carga y conviene decidirlo con tests de regresión antes — coincide con la recomendación del propio informe de no tocar esto sin antes endurecer el mapeo.

He validado con una prueba aislada (sin gmsh real) que el bug original efectivamente rompe (IndexError) y que la lógica corregida se comporta de forma segura. El archivo completo corregido está arriba (face_correspondence.py) — reemplaza el que está en core/ de tu repo. Como siguiente paso, valdría la pena añadir los tests que el propio informe pedía (caras simétricas, fallo parcial de getValue, correspondencia biyectiva, rechazo de ambigüedad) — puedo escribirlos si quieres.