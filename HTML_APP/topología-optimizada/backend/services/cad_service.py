...
class CADService:
    ...
    def list_solids(self, model_id: str) -> List[Dict[str, Any]]:
        ...
        "faces_count", "center", "name", "face_indices"}``.
        """
        # MULTI-VIEW (reversible): "face_indices" son los face_index GLOBALES
        ...
        try:
            ...
            for idx, solid in enumerate(solids):
                ...
                try:
                    ...
                    faces_count = 0
                # MULTI-VIEW (reversible): caras globales del solido.
                try:
                    sfaces = list(solid.Faces())
                    face_indices = []
                    for j, gf in enumerate(global_faces):
                        for sf in sfaces:
                            try:
                                same = bool(gf.wrapped.IsSame(sf.wrapped))
                            except Exception:
                                same = False
                            if same:
