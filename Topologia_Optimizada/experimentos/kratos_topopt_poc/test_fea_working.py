#!/usr/bin/env python3
"""
Script simplificado para FEA básico usando API directa de Kratos pero corregido
...
def run_corrected_fea():
    """
    Ejecuta análisis FEA básico de viga en voladizo usando API directa de Kratos con correcciones
    """
    
    print("=== INICIANDO ANÁLISIS FEA CORREGIDO ===")
    ...
    if element_type_3d:
        ...
        if tet_elements is not None:
            ...
            try:
                ...
                # Intentar registro
                try:
                    constitutive_law = Kratos.KratosGlobals.GetRegistry("LinearElastic3DLaw")
                    material_properties.SetValue(Kratos.CONSTITUTIVE_LAW, constitutive_law)
                    print("Ley constitutiva configurada vía registro")
                except Exception as e2:
                    print(f"Error con registro: {e2}")
                    # Crear ley constitutiva genérica
                    try:
                        constitutive_law = StructuralMechanicsApplication.SmallStrainIsotropic3D()
                        material_properties.SetValue(Kratos.CONSTITUTIVE_LAW, constitutive_law)
                        print("Ley constitutiva genérica configurada")
                    except Exception as e3:
                        print(f"Error con ley genérica: {e3}")
                        raise Exception("No se pudo configurar ninguna ley constitutiva")
...
if __name__ == "__main__":
    success, results = run_corrected_fea()
